   ...
   fDetectorConstruction(Det)
{
  ...
  // fFilenameCmd = new G4UIcmdWithAString("/B2/setFilename",this);
  ...
  // fNormFactorCmd = new G4UIcmdWithADouble("/B2/normFactor",this);
  // fNormFactorCmd->SetGuidance("Set normalisation factor.");
  // fNormFactorCmd->SetParameterName("normFactor",false);
  // fNormFactorCmd->AvailableForStates(G4State_Idle);

  // fAreaCmd = new G4UIcmdWithADouble("/B2/area",this);
  // fAreaCmd->SetGuidance("Set particle source area.");
  // fAreaCmd->SetParameterName("area",false);
  // fAreaCmd->AvailableForStates(G4State_Idle);
}
...
void B2aDetectorMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  ...
  //  { fDetectorConstruction->SetShellMaterial(newValue);}
  ...
  // if( command == fNormFactorCmd ) {
  //   runAction->SetNormFactor(fNormFactorCmd->GetNewDoubleValue(newValue));
  // }

  // if( command == fAreaCmd ) {
  //   runAction->SetArea(fAreaCmd->GetNewDoubleValue(newValue));
  // }
}
